...
The crawler dispatches to the user-selected engine (see ADR-0003). The **Rod** engine spawns a headless Chromium instance via DevTools Protocol, the **Colly** engine uses pure HTTP requests, and the **ZAP** engine delegates to an OWASP ZAP sidecar. All engines navigate to the entry URL. If auth is configured, the engine performs the auth flow: form login fills username and password fields (located by heuristics: input[type=text] or input[type=email] for username, input[type=password] for password, button[type=submit] or input[type=submit] for the submit in Rod; via POST in Colly and ZAP), bearer auth injects the Authorization header, and basic auth injects the encoded credentials header.
